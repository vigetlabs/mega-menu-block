...
class GitHub_Plugin_Updater {
	...
	public function plugin_info( $result, $action, $args ) {
		...
		return (object) [
			...
			'short_description' => esc_html__( 'Latest version from GitHub', 'mega-menu-block' ),
			'sections'          => [
	...
	private function format_changelog( $changelog ) {
		if ( empty( $changelog ) ) {
			return esc_html__( 'No changelog available.', 'mega-menu-block' );
		}
		...
		$changelog = preg_replace_callback(
			...
			static function ( $matches ) use ( &$link_placeholders ) {
				$placeholder = '__MMB_LINK_' . count( $link_placeholders ) . '__';
	...
	private function get_transient_key() {
		return 'mmb_github_updater_' . md5( $this->github_owner . '/' . $this->github_repo );
	}
